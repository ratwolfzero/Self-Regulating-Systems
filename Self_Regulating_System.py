...
def instantaneous_frequency(x, t, epsilon):
    ...
    if epsilon > 0.1:
        print(
            f"Warning: Large epsilon ({epsilon}) may violate Hilbert transform narrowband assumption.")
    return freq
...
def prepare_plotting_data(t, x_s, x_f, start_time, end_time, epsilon_success, epsilon_fail):
    ...
    start_idx, end_idx = get_convergence_window_indices(
        t, start_time, end_time)
    t_window = t[start_idx:end_idx]
    freq_s = instantaneous_frequency(
        x_s[start_idx:end_idx], t_window, epsilon_success)
    freq_f = instantaneous_frequency(
...
def simulate_case(epsilon, theta_tol=theta_tol):
    if epsilon > 0.1:
        print(
            f"Using solve_ivp for epsilon={epsilon} to ensure numerical stability")
        sol = solve_ivp(
        ...
        sol = odeint(adaptive_oscillator, x0, t,
                     args=(epsilon,), rtol=1e-6, atol=1e-8)
        x, v, theta = sol.T
    ...
    damped_freq = np.sqrt(max(mean_theta - (delta**2 / 4), 0)) / (2 * np.pi)
...
def plot_stacked(t, theta_s, theta_f, x_s, x_f, energy_s, energy_f,
    ...
    plt.plot(t, theta_s, color=color_s,
             label=f'Theta s (ε={eps_s:.4f}, {status_s})')
    plt.plot(t, theta_f, color=color_f, linestyle='--',
             label=f'Theta f (ε={eps_f:.4f}, {status_f})')
    plt.xlabel('Time (s)')
    plt.ylabel('Theta')
    ...
    plt.plot(t, x_f, color=color_f, linestyle='--',
             label=f'x f (ε={eps_f:.4f}, {status_f})')
    plt.xlabel('Time (s)')
    plt.ylabel('x')
    plt.title('Position')
    plt.legend()
    ...
    plt.subplot(3, 1, 3)
    plt.axvspan(*convergence_window, color='gray', alpha=0.2)
    plt.plot(t, energy_s, color=color_s,
             label=f'Energy s (ε={eps_s:.4f}, {status_s})')
    plt.plot(t, energy_f, color=color_f, linestyle='--',
             label=f'Energy f (ε={eps_f:.4f}, {status_f})')
    plt.xlabel('Time (s)')
    plt.ylabel('Energy')
    plt.title('Energy')
    plt.legend()
...
def plot_phase_space(x_s, v_s, x_f, v_f, eps_s, eps_f, status_s, status_f):
    ...
    plt.plot(x_s, v_s, color=color_s,
             label=f'Phase s (ε={eps_s:.4f}, {status_s})')
    plt.plot(x_f, v_f, color=color_f, linestyle='--',
             label=f'Phase f (ε={eps_f:.4f}, {status_f})')
    plt.xlabel('x')
    plt.ylabel('v')
...
def plot_instantaneous_frequency(t, freq_s, freq_f, eps_s, eps_f, status_s, status_f):
    ...
    plt.plot(t, freq_s, color=color_s,
             label=f'IF s (ε={eps_s:.4f}, {status_s})')
    plt.plot(t, freq_f, color=color_f, linestyle='--',
             label=f'IF f (ε={eps_f:.4f}, {status_f})')
    plt.xlabel('Time (s)')
...
if __name__ == "__main__":
    # Run simulations
    x_s, v_s, theta_s, energy_s, status_s, mean_theta_s, std_theta_s, mean_freq_s, damped_freq_s, tol_s = simulate_case(
        epsilon_success)
    x_f, v_f, theta_f, energy_f, status_f, mean_theta_f, std_theta_f, mean_freq_f, damped_freq_f, tol_f = simulate_case(
        epsilon_fail)
    ...
    t_window, freq_s, freq_f = prepare_plotting_data(
        t, x_s, x_f, start_time, end_time, epsilon_success, epsilon_fail)
